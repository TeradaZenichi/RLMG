...
from .model import make_sac_nets  # ator constrained (model2)
...
class TrainConfig:
    ...
    # paths
    model_dir: str = "models/model2"
    model_json: str = str(Path(__file__).resolve().parent / "model.json")
    ckpt_dir: str = "saves/model2"
...
def train(cfg: TrainConfig = TrainConfig()):
    ...
    safety_temp = float(net_hp.get("safety_temp", 1.0))  # específico do model2
    ...
    # >>> PASSAR CONTEXTO DO AMBIENTE PARA O ATOR (usar dt do treino!)
    env_ctx = {
        "PV_Pmax_kw": float(env_cfg.PV_Pmax_kw),
        "Load_Pmax_kw": float(env_cfg.Load_Pmax_kw),
        "E_bess_kwh": float(env_cfg.E_bess_kwh),
        "eta_ch": float(env_cfg.eta_ch),
        "eta_dis": float(env_cfg.eta_dis),
        "soc_min": float(env_cfg.soc_min),
        "soc_max": float(env_cfg.soc_max),
        "P_ch_max_kw": float(env_cfg.P_ch_max_kw),
        "P_dis_max_kw": float(env_cfg.P_dis_max_kw),
        "dt_minutes": int(cfg.dt_minutes),  # << dt do treino
        "grid_import_kw_max": float(getattr(env_cfg, "grid_import_kw_max", float("inf"))),
        "grid_export_kw_min": float(getattr(env_cfg, "grid_export_kw_min", float("-inf"))),
    }
    nets.actor.set_env_context(SimpleNamespace(**env_ctx))

    actor.train()
    critic.train()
    target_critic.eval()  # usado só para alvo
    ...
    while env_steps < cfg.total_env_steps:
        ...
        with torch.no_grad():
            ...
            if rb.size < learning_starts:
                ...
                # >>> garante shape (n_envs, act_dim)
                actions = a.view(-1, act_dim).cpu().numpy().astype(np.float32)
        ...
        if rb.size >= learning_starts:
            ...
            for g in range(updates):
                ...
                # >>> segurança: (B, act_dim)
                if b_act.dim() == 1:
                    b_act = b_act.unsqueeze(1)
                else:
                    b_act = b_act.view(-1, act_dim)

                with torch.no_grad():
                    a_next, logp_next, _ = actor.sample(b_next, deterministic=False)   # [B, A], [B]
                    a_next = a_next.view(-1, act_dim)                                  # <<< garante (B,1)
                ...
                a_pi = a_pi.view(-1, act_dim)                                          # <<< garante (B,1)
        ...
        if cfg.checkpoint_every and (env_steps % cfg.checkpoint_every) == 0:
            ...
            env_ctx_ckpt = dict(env_ctx)  # já contém dt_minutes = cfg.dt_minutes
            torch.save({
